...
app.post('/api/checkAvailable', (req, res) => {
    (async function() {
		...
		console.log(items[i]);
		var resourceResult = await runQuery('select resource_id from service where id = ' + items[i].service_id);

                query = 'select e.* from event e, service s where s.id = e.service_id and e.resource_id = ' + resourceResult.rows[0].resource_id + ' and extract(epoch from e.date) between ' +
